...
pub async fn mount_f3bfs_daemonize<F>(
    mountpoint: String,
    endpoints: Vec<&str>,
    ...
    F: FnOnce() -> anyhow::Result<()>,
{
    let mut fuse_options = vec![
        FuseMountOption::FSName(format!("f3bfs:{}", endpoints.join(","))),
...
pub async fn mount_f3bfs(
    mountpoint: String,
    endpoints: Vec<&str>,
    options: Vec<MountOption>,
) -> anyhow::Result<()> {
    mount_f3bfs_daemonize(mountpoint, endpoints, options, || Ok(())).await
}
